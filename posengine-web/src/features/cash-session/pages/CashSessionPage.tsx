...
import { Wallet, Lock, Unlock, History, Info, AlertCircle, CheckCircle2 } from 'lucide-react'
import { useCashSession } from '../hooks/useCashSession'
import { OpenCashModal } from '../components/OpenCashModal'
import { CloseCashModal } from '../components/CloseCashModal'
import { HistoryModal } from '../components/HistoryModal'
import { CashClosedInfoModal } from '../components/CashClosedInfoModal'
import { Header } from '@/src/shared/components/Header'

import styles from './CashSessionPage.module.css'

export function CashSessionPage() {
  const { session, closedToday, loading, error, openCash, closeCash, forceCloseCash, refresh } = useCashSession()
  ...
  const handleForceClose = async (amount: number, confirmation: string, reason?: string) => {
    setActionLoading(true)
    try {
      await forceCloseCash(session!.id, amount, confirmation, reason)
      setIsClosing(false)
    } finally {
      setActionLoading(false)
    }
      ...
      <div className={styles.container}>
        ...
        {session ? (
            ...
            <div className={styles.sessionGrid}>
              ...
              <div className={styles.statsGrid}>
                ...
                <div className={`${styles.statItem} ${styles.statTotal}`}>
                  ...
                  <span className={styles.statValueLarge}>{formatGs(session.openingAmount + session.totalSales - session.totalPurchases)}</span>
                </div>
              </div>
            </div>
          ...
          <div className={styles.emptyState}>
            ...
            <h2>Caja completada por hoy</h2>
            <p>Ya se ha realizado el cierre de caja del día de hoy. Podrás realizar una nueva apertura mañana.</p>
            <div className={styles.summaryNote}>
              El resumen de hoy está disponible en el historial de sesiones.
            </div>
            <button
            ...
            >
              ¿Por qué no puedo reabrir la caja?
            </button>
          </div>
        ) : (
          /* ESTADO: CAJA PENDIENTE DE APERTURA */
          <div className={styles.emptyState}>
            <div className={styles.emptyIconWrap}>
              <Wallet size={48} className={styles.mutedIcon} />
            </div>
            <h2>Caja cerrada</h2>
            <p>Debes realizar la apertura de caja para comenzar a registrar movimientos y ventas.</p>
            <button 
